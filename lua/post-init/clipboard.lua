...
if #clipboard_providers == 0 and vim.g.neoteusz_clipboard_skip ~= true then
    ...
    async.run(function ()
      vim.notify("No clipboard providers found, either install a compatible clipboard provider or set the `clipboard.skip` field within the config.lua file true to ignore clipboard errors. To see compatible clipboards, use the `help: g:clipboard` command (or telescope help search).", vim.log.levels.WARN, {title = vim.g.neoteusz_name})
